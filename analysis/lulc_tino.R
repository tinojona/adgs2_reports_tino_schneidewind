...
# ADD DATA FROM KOEN HERE


set.seed(123)  
split <- rsample::initial_split(train, prop = 0.7, strata = "LC1")
train_train <- rsample::training(split)
train_test <- rsample::testing(split)
...
# hyper-parameters
xgb_best_model <- fit(xgb_best_hp, train_train[,-c(1,3,4)]) # remove lon lat PixelID


# run the model on our test data
# using predict()
test_results <- predict(xgb_best_model, train_test[,-c(1,3,4)])


# if results are better then koens
test_results <- predict(xgb_best_model, test)

# load the caret library to
# access confusionMatrix functionality
test_results = read.csv("./data/LULC_tino.csv")
write.csv(test_results[,2], file = "./data/tinojona_newresults.csv", row.names = F, quote = F)
