...
As data, we used PhenoCam data, which we accessed using the [{phenocamr}](https://bluegreen-labs.github.io/phenocamr/) [R package](https://bluegreen-labs.github.io/phenocamr/). We chose the site "harvard" in north eastern USA. The data was downloaded according to chapter 6.2. The used model for GDD and the model optimisation method is described in chapter 6.2. After optimising the model, we have values for the **temperature threshold (2.811523)** and the **number of accumulation days (228.27844)**.


```{r harvard_data, echo=FALSE, message=FALSE}
...
```{r harvard_processing, echo=FALSE, message=FALSE}
...
```{r model_optimisation, echo=FALSE, message=FALSE}
...
The model and the observed leaf out days had visually had a similar tendency, however, the slope between both was only 0.74 instead of the optimal of 1. The linear model of both datasets was significant with p = 0.005, with an  Rsquared of 0.46.


We scaled the model spatially for the year 2010 to compare it later on.

```{r harvard_spatial, echo=FALSE, message=FALSE, results="hide"}
...
1. One could add data **additional training data** in order to improve the model generelisability. I will implement this method further down in this report. 
2. It would be possible to improve the model by **adding another parameter** such as radiation, humidity or soil propoerties. This would make the model more sensitive to non temperature abiotic factors.
...
4. Of course, we could implement **machine learning** to improve our predictions, however, this would be considered a change of model and not an optimization

<br>



#### 1. Implementation of Optimization
In order to ensure some comparibility between the new site and the harvard site, I chose a site nearby. The **worchester** site (lat: 42.2697, lon: -71.8428) is approximately 40 km southeast of the harvard site. Compared to harvard (340 m above sea level), worchester has less elevation (185 m above sea level). This site is also part of the PhenCam network. Both datasets were given an ID, so that we are able to distinguish between the two sites in the modeling process.  

```{r worchester, echo=FALSE, message=FALSE}
phenocamr::download_phenocam(
  site = "worcester",
  veg_type = "DB",
  roi_id = "1000",
...
```{r worchester_processing, echo=FALSE, message=FALSE}
...
```{r optimizationdoubble, echo=FALSE, message=FALSE}
data_ha_wo <- list(
...
In this plot you can see, that my improved model has pretty good alignement with the validation data. However, there is the clear outlier, which was predicted in 2013. This outlier did not appear before we adjusted the model by adding the worchester data. The slope of the linear model improved up to 0.82. However, Rquared is now only 0.1 and p = 0.23. The significance was likely lost due to the outlier. 

```{r modworchester, echo=FALSE}
print(predictions_ha_wo)

lm(predictions_ha_wo$prediction ~ harvard_phenology$doy)$coeff
# summary(lm(predictions_ha_wo$prediction ~ harvard_phenology$doy))
...
```{r harvard_wor_spatial, echo=FALSE, message=FALSE}
...
```{r scale harvard_wor, echo=FALSE, message=FALSE}
pal <- colorNumeric(
  "magma",
...
### 2. Statistical Comparison to MODIS Data 
...
In order to compare our results from the modified model with the MODIS MCD12Q2 phenology product, we needed to download it first. First, I downloaded the data from MODIS for the harvard site, and spatially averaged the yearly leaf out dates of the 20km^2 raster to get values, that can be compared to our predicted values. 

<br>

#### Comparison over all years

```{r load_modis_harvard, echo=FALSE}
...
```{r modis_harvard_processing, echo=FALSE}
phenology <- phenology |>
  mutate(
...
```{r modis_worchester_processing, echo=FALSE}
phenology <- phenology |>
  mutate(
    value = ifelse(value > 32656, NA, value),
    value = as.numeric(format(as.Date("1970-01-01") + value, "%j")),
...
```{r statistical_comparison, echo=FALSE}
t.test(predictions_ha_wo$prediction, phenology_filtered$mean_2)
# cor(predictions_ha_wo$prediction, phenology_filtered$mean_2) 
```


```{r stats_plots, echo=FALSE, message=FALSE}
...
MODIS raster (I don't know why the plot is tilted):

```{r modis_2010, echo=FALSE, message=FALSE}
phenology <- MODISTools::mt_subset(
  product = "MCD12Q2",
  lat = 42.5378,
  lon = -72.1715,
...
```{r crop_compare, echo=FALSE, message=FALSE}
...
Overall, we saw a poor agreement of the modeled predictions and the observed leaf out dates. This is likely due to 2010 being an extraordinary year in term of temperature. Average temperature was observed to be slightly above average. Important for our predictions, the winter was abnormally cold and spring record warm. (source: [NCEI USA](https://www.ncei.noaa.gov/access/monitoring/monthly-report/national/201013)) This rapid warming influences the warming days we tried to model, leading to possibly wrong conclusions as the system operates in the extremes.  
